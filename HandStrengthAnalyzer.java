...
public class HandStrengthAnalyzer{
  private ArrayList<Card> kicker = new ArrayList<>();
  private ArrayList<Card> flushList = new ArrayList<>();
  private int[] setStrengthArray;
  private int   handStrengthVal = 0,
                highSet = 0,
                lowSet = 0,
                highSetIdx = 0,
                lowSetIdx = 0,
  ...
  private int checkFlush(ArrayList<Card> hand){
    ...
    for(int i = 0; i < hand.size()-1; i++){
      ...
      if(c1==5){flushSuit = hand.get(i).getSuitNumber();}
      if(c1 == 5){
          fl = 1;
      }
    }
    ...
    sortArrayListRank(hand);
     ...
     for(int hs = this.setStrengthArray.length - 1; hs >= 0; hs--){        
      ...
      for(int tl = 0; tl < hand.size(); tl++){
        if((hand.get(tl).getRankNumber() == currCardInt && hs ==
            (this.setStrengthArray.length - 1)) ||
  ...
  private int checkSets(ArrayList<Card> hand){
    ...
    for(int i = 0; i < this.setStrengthArray.length; i++){
      ...
      if(setStrengthArray[i] == 4){
        incrementVal = 9999;} else {incrementVal = setStrengthArray[i];}
      setStrengthVal += incrementVal;
    }//Set four of a kind to a distinct value so that it is not confused with two pair.

    if(setStrengthVal == 2)
    {setStrengthVal = 1;} else
      if(setStrengthVal >=4 && setStrengthVal % 2 == 0)
    {setStrengthVal = 2;} else
      if(setStrengthVal >= 5 && setStrengthVal % 2 == 1)
